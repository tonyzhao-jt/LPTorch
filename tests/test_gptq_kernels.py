...
from lptorch.gptq.functional import vecquant2matmul_torch, vecquant3matmul_torch, vecquant4matmul_torch, vecquant8matmul_torch
...
tick = time.time()
for _ in range(COUNT):
    vecquant2matmul_torch(vec, mat, mul, scales, zeros, M)
    torch.cuda.synchronize()
print('2bit:', (time.time() - tick) / COUNT)

tick = time.time()
for _ in range(COUNT):
    vecquant3matmul_torch(vec, mat, mul, scales, zeros, M)
    torch.cuda.synchronize()
print('3bit:', (time.time() - tick) / COUNT)

tick = time.time()
for _ in range(COUNT):
    vecquant4matmul_torch(vec, mat, mul, scales, zeros, M)
    torch.cuda.synchronize()
print('4bit:', (time.time() - tick) / COUNT)

tick = time.time()
for _ in range(COUNT):
    vecquant8matmul_torch(vec, mat, mul, scales, zeros, M)
    torch.cuda.synchronize()
